# SKILL: Virtual Filesystem Tool Unification

Use this pattern when: Agent needs to interact with 5+ heterogeneous backends (S3, Postgres, Slack, GitHub, LanceDB) and accumulates N different SDK integrations, each with its own auth pattern, error model, and tool schema, making the tool manifest bloated and the agent context polluted with low-level API vocabulary
